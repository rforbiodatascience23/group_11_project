...
```{r}
variables_to_plot <- prostate_data_augment[, c("Primary lesion size(cm^2)", "age", "Systolic blood pressure", "Diastolic blood pressure")]

# Reshape the data for ggplot
data_long <- gather(variables_to_plot, key = "Variable", value = "Value")
...
pca_fit <- prostate_data_augment |>
  select(`Treatment regime`, `age`, `Primary lesion size(cm^2)`, `Gleason stage`, `Serum haemoglobin level`, `stage`,`History of CVD`, `Activity Level`)|>
  prcomp(scale=TRUE) 
...
#pca_fit <- prostate_data_augment |>
#  select(`Treatment regime`,`stage`,`age`, `Serum haemoglobin level`, `Primary lesion size(cm^2)`,`Systolic blood pressure`, )|>
#  prcomp(scale=TRUE) 
...
ggsave('../results/pca_01.png', pca_fit)
```
